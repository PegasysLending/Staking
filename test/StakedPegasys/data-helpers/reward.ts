...
export const compareRewardsAtAction = async (
  ...
  const rewardsBalanceBefore = BigNumber.from(
    await (await stakedPegasys.getTotalRewardsBalance(userAddress)).toString()
  );
  ...
  const assetConfiguration = assetConfig
    ...
    : {
        ...
        totalStaked: await stakedPegasys.totalSupply(),
        underlyingAsset,
      };
  await stakedPegasys.configureAssets([assetConfiguration]);
...
export const compareRewardsAtTransfer = async (
  ...
  const underlyingAsset = stakedPegasys.address;
  const fromSavedBalance = await stakedPegasys.balanceOf(fromAddress);
  const toSavedBalance = await stakedPegasys.balanceOf(toAddress);
  const fromSavedRewards = BigNumber.from(
    await (await stakedPegasys.getTotalRewardsBalance(fromAddress)).toString()
  );
  const toSavedRewards = BigNumber.from(
    await (await stakedPegasys.getTotalRewardsBalance(toAddress)).toString()
  );
  // Get index before actions
  const fromIndexBefore = await getUserIndex(stakedPegasys, fromAddress, underlyingAsset);
  const toIndexBefore = await getUserIndex(stakedPegasys, toAddress, underlyingAsset);

  // Load actions that can or not update the user index
  const actions = () => [stakedPegasys.connect(from.signer).transfer(toAddress, amount)];

  // Fire reward comparator
  await compareRewardsAtAction(stakedPegasys, fromAddress, actions, fromShouldReward, assetConfig);

  // Check rewards after transfer

  // Get index after actions
  const fromIndexAfter = await getUserIndex(stakedPegasys, fromAddress, underlyingAsset);
  const toIndexAfter = await getUserIndex(stakedPegasys, toAddress, underlyingAsset);
  ...
  if (fromAddress === toAddress) {
    ...
    const fromNewBalance = await (await stakedPegasys.balanceOf(fromAddress)).toString();
    const toNewBalance = await (await stakedPegasys.balanceOf(toAddress)).toString();
