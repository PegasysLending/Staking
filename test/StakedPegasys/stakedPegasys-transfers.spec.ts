...
makeSuite('StakedPegasys. Transfers', (testEnv: TestEnv) => {
  ...
  it('User 1 transfers 50 SSYS to User 5', async () => {
    ...
    await compareRewardsAtTransfer(stakedPegasys, sender, receiver, amount, true, false);
  });
  ...
  it('User 5 transfers 50 SSYS to himself', async () => {
    ...
    await compareRewardsAtTransfer(stakedPegasys, sender, sender, amount, true, true);
  });
  ...
  it('User 4 stakes and transfers 50 SSYS to user 2, with rewards not enabled', async () => {
    ...
    const actions = () => [
      pegasysToken.connect(sender.signer).approve(stakedPegasys.address, amount),
      stakedPegasys.connect(sender.signer).stake(sender.address, amount),
    ];

    await compareRewardsAtAction(stakedPegasys, sender.address, actions, false, assetConfig);
    await compareRewardsAtTransfer(
      stakedPegasys,
      sender,
      receiver,
  ...
  it('Activate cooldown of User2, transfer entire amount from User2 to User3, cooldown of User2 should be reset', async () => {
    ...
    const amount = await stakedPegasys.balanceOf(sender.address);
    ...
    await stakedPegasys.connect(sender.signer).cooldown();
    const cooldownActivationTimestamp = await (await timeLatest()).toString();

    const cooldownTimestamp = await stakedPegasys.stakersCooldowns(sender.address);
    ...
    const cooldownTimestampAfterTransfer = await (
      await stakedPegasys.stakersCooldowns(sender.address)
    ).toString();
  ...
  it('Transfer balance from User 3 to user 2 cooldown  of User 2 should be reset if User3 cooldown expired', async () => {
    ...
    await pegasysToken.connect(receiver.signer).approve(stakedPegasys.address, amount);
    await stakedPegasys.connect(receiver.signer).stake(receiver.address, amount);
    await stakedPegasys.connect(receiver.signer).cooldown();
    const receiverCooldown = await stakedPegasys.stakersCooldowns(sender.address);
    ...
    const stakerCooldownTimestampBefore = await stakedPegasys.stakersCooldowns(receiver.address);
  ...
  it('Transfer balance from User 3 to user 2, cooldown of User 2 should be the same if User3 cooldown is less than User2 cooldown', async () => {
    ...
    const receiverCooldown = await (
      await stakedPegasys.stakersCooldowns(receiver.address)
    ).toString();
