...
export const compareRewardsAtAction = async (
  ...
  const rewardsBalanceBefore = BigNumber.from(
    await (await stakedPegasysV2.getTotalRewardsBalance(userAddress)).toString()
  );
  ...
  const assetConfiguration = assetConfig
    ...
    : {
        ...
        totalStaked: await stakedPegasysV2.totalSupply(),
        underlyingAsset,
      };
  await stakedPegasysV2.configureAssets([assetConfiguration]);
...
export const compareRewardsAtTransfer = async (
  ...
  const underlyingAsset = stakedPegasysV2.address;
  const fromSavedBalance = await stakedPegasysV2.balanceOf(fromAddress);
  const toSavedBalance = await stakedPegasysV2.balanceOf(toAddress);
  const fromSavedRewards = BigNumber.from(
    await (await stakedPegasysV2.getTotalRewardsBalance(fromAddress)).toString()
  );
  const toSavedRewards = BigNumber.from(
    await (await stakedPegasysV2.getTotalRewardsBalance(toAddress)).toString()
  );
  // Get index before actions
  const fromIndexBefore = await getUserIndex(stakedPegasysV2, fromAddress, underlyingAsset);
  const toIndexBefore = await getUserIndex(stakedPegasysV2, toAddress, underlyingAsset);

  // Load actions that can or not update the user index
  const actions = () => [stakedPegasysV2.connect(from.signer).transfer(toAddress, amount)];
  ...
  // Get index after actions
  const fromIndexAfter = await getUserIndex(stakedPegasysV2, fromAddress, underlyingAsset);
  const toIndexAfter = await getUserIndex(stakedPegasysV2, toAddress, underlyingAsset);
  ...
  if (fromAddress === toAddress) {
    ...
    const fromNewBalance = await (await stakedPegasysV2.balanceOf(fromAddress)).toString();
    const toNewBalance = await (await stakedPegasysV2.balanceOf(toAddress)).toString();
