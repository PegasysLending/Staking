...
makeSuite('StakedPegasysV2. Permit', (testEnv: TestEnv) => {
  it('Reverts submitting a permit with 0 expiration', async () => {
    const { deployer, users, stakedPegasysV2 } = testEnv;
    ...
    const expiration = 0;
    const nonce = (await stakedPegasysV2._nonces(owner)).toNumber();
    ...
    expect((await stakedPegasysV2.allowance(owner, spender)).toString()).to.be.equal(
      '0',
  ...
  it('Submits a permit with maximum expiration length', async () => {
    ...
    expect((await stakedPegasysV2._nonces(owner)).toNumber()).to.be.equal(1);
  });
  ...
  it('Cancels the previous permit', async () => {
    ...
    expect((await stakedPegasysV2._nonces(owner)).toNumber()).to.be.equal(2);
  });
