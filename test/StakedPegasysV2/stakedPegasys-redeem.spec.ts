...
makeSuite('StakedPegasys V2. Redeem', (testEnv: TestEnv) => {
  it('Reverts trying to redeem 0 amount', async () => {
    ...
    await expect(
      stakedPegasysV2.connect(staker.signer).redeem(staker.address, amount)
  ...
  it('User 1 stakes 50 SYS', async () => {
    ...
    await waitForTx(
      await pegasysToken.connect(staker.signer).approve(stakedPegasysV2.address, amount)
    );
    await waitForTx(await stakedPegasysV2.connect(staker.signer).stake(staker.address, amount));
  });
  ...
  it('User 1 activates the cooldown, but is not able to redeem before the COOLDOWN_SECONDS passed', async () => {
    ...
    const startedCooldownAt = new BigNumber(
      await (await stakedPegasysV2.stakersCooldowns(staker.address)).toString()
    );
  ...
  it('User 1 activates the cooldown again, and tries to redeem a bigger amount that he has staked, receiving the balance', async () => {
    ...
    const stakedPegasysBalanceBefore = (await stakedPegasysV2.balanceOf(staker.address)).toString();
    await stakedPegasysV2.connect(staker.signer).redeem(staker.address, amount);
    const pegasysBalanceAfter = new BigNumber(
      (await pegasysToken.balanceOf(staker.address)).toString()
    );
    const stakedPegasysBalanceAfter = (await stakedPegasysV2.balanceOf(staker.address)).toString();
