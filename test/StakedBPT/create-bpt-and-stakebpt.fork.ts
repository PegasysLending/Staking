...
import { testDeployPegasysStakeV1 } from '../helpers/deploy';

const { expect } = require('chai');

const WETH_HOLDER = '0x1840c62fD7e2396e470377e6B2a833F3A1E96221';
// const SYS_WETH_HOLDER = '0x7d439999e63b75618b9c6c69d6efed0c2bc295c8';
...
const INIT_SYS_PRICE = 502; // 1 ETH = 5.14 SYS
...
rawBRE.run('set-dre').then(async () => {
  makeSuite('Deploy stkBPT', async (testEnv: TestEnv) => {
    ...
    let holderPegasysBalance: ethers.BigNumber;
    ...
    before(async () => {
      ...
      pegasys = await getERC20Contract(SYS_TOKEN);
    ...
    beforeEach(async () => {
      ...
      holderPegasysBalance = await pegasys.balanceOf(MULTI_SIG);
    });
    ...
    it('Creates the smart Pool: 80/20 SYS/ETH', async () => {
      await waitForTx(
        await pegasys.connect(holderSigner).approve(CRPool.address, INFINITE_APPROVAL_AMOUNT)
      );
      await waitForTx(
      ...
      expect(await pegasys.balanceOf(MULTI_SIG)).to.be.equal(
        holderPegasysBalance.sub(INIT_SYS_POOL_SUPPLY)
      );
    ...
    it('Let a user make a swap Weth => Pegasys', async () => {
      await waitForTx(
        await pegasys.connect(holderSigner).approve(BPool.address, INFINITE_APPROVAL_AMOUNT)
      );
      await waitForTx(
        await weth.connect(holderSigner).approve(BPool.address, INFINITE_APPROVAL_AMOUNT)
      );
      // swapping weth for pegasys. Price is currently 1 SYS = 10 ETH
      ...
      await waitForTx(
        await BPool.connect(holderSigner).swapExactAmountIn(
          ...
          SYS_TOKEN,
          parseEther('0.00001'),
          parseEther('0.0005').mul(INIT_SYS_PRICE)
        )
      );
      expect(await weth.balanceOf(MULTI_SIG)).to.be.equal(holderWethBalance.sub(SOLD_WETH));
    });
    it('Let a user make a swap SYS => WETH', async () => {
      const SOLD_SYS = parseEther('0.005');
      await waitForTx(
        await BPool.connect(holderSigner).swapExactAmountIn(
          SYS_TOKEN,
          SOLD_SYS,
      ...
      expect(await pegasys.balanceOf(MULTI_SIG)).to.be.equal(holderPegasysBalance.sub(SOLD_SYS));
    });
    ...
    it('Stake shares and get rewards', async () => {
      ...
      expect(await pegasys.balanceOf(REWARDS_RECEIVER)).to.be.equal(1);
    });
  });
});
