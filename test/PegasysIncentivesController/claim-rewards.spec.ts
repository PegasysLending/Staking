...
makeSuite('PegasysIncentivesController claimRewards tests', (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      const { pegasysIncentivesController, stakedPegasys, pegasysToken, aDaiMock } = testEnv;
      ...
      const destinationAddressBalanceBefore = await (toStake
        ? stakedPegasys
        : pegasysToken
      ...
      const destinationAddressBalanceAfter = await (toStake
        ? stakedPegasys
        : pegasysToken
      ).balanceOf(destinationAddress);
